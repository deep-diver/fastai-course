...
[**몽키 패치**](https://en.wikipedia.org/wiki/Monkey_patch)는 런타임시 이미 존재하는 모듈에 기능을 얹는 프로그래밍 기법입니다. 사용성이 좋고, 빠르게 기존 프로그램을 확장할 수 있지만 남용해서는 안됩니다. 빠르게 새로운 기능을 기존 모듈에 탑재해서 테스트 해보는데 용이하게 사용될 수 있으며, 모든 기능이 안정성 있게 구동됨을 확인한 후에는 **상속**과 같은 방법으로 체계를 갖추어 구현하는것이 바람직합니다. 대체로 그 이유는 **몽키 패치**가 남용될 경우 코드의 가독성이 매우 떨어지며, 코드에 내재된 작동 방식을 잘못 이해하는 경우가 종종 발생하기 때문입니다.

fastcore 라이브러리는 **몽키 패치**를 위한 보다 간단한 방법으로 `@patch` 데커레이션을 제공합니다. 단순히 특정 함수에 `@patch` 데커레이션을 붙이고, 첫 번째 인자로는 확장하고자 하는 **클래스**를 `self`로서 제공하기만 하면 됩니다. 또한 추가적으로 `as_prop` 옵션을 사용하면 `@property` 효과를, `cls_method` 옵션을 사용하면 `@classmethod` 효과를 얻을수도 있죠. 간단한 예를 살펴보죠.

아래의 코드를 완성해보세요. `Person`는 사람을 표현하는 클래스로, 이름(**firstname**), 성(**lastname**), 나이(**age**) 라는 속성을 가지고 있습니다. 이후 사람 객체를 반환하는 팩터리 메서드인 `CreatePerson`, 나이를 한 살 더 추가하는 `grow_older` 메서드, 이름과 성을 이어 붙여서 **계산된** 속성을 반환하는 `full_name` 을 만들어보기로 했다는 가정입니다. 새롭게 추가할 세 개는 각각 **클래스 메서드**, **기본 메서드**, **속성** 이라는점을 감안해서 코드를 완성해보기 바랍니다. 
...
객체지향 프로그래밍에서 흔히 접하는 개념 중 하나는 [메서드 오버로딩(Method Overloading)](https://ko.wikipedia.org/wiki/%ED%95%A8%EC%88%98_%EC%98%A4%EB%B2%84%EB%A1%9C%EB%93%9C) 입니다. 서로 다른 자료형의 파라미터와 반환 값을 가지는 동일한 이름의 함수/메서드를 여러개 정의할 수 있다는 개념이죠. 파이썬에서는 메서드 오버로딩이 기본적으로 지원되지 않습니다. 즉 특정 이름을 갖는 함수/메서드는 하나만 정의할 수 있습니다. 

기본 파이썬에서는 메서드 오버로딩이라는 개념 대신, 디폴트 값 설정 메커니즘을 활용합니다. 다양한 파라미터를 두고, 값이 있을때만 처리하겠다는 뜻으로 해석될 수 있습니다. 하지만 파라미터의 개수가 너무 많아지면, 가독성이 떨어지고 관리가 어려워진다는 문제가 있기 때문에 초기에 설계를 잘 해야하죠. fastcore 패키지에서는 `타입 디스패치` 라는 개념을 파이썬에서도 사용할 수 있는 간단한 시스템을 구현했습니다. 간단히는 함수 오버로딩이라고 생각해도 좋아요.

같은 이름을 가진 함수를 여러개 정의해두고, 입려되는 파라미터의 자료 형에따라 달리 작동하게끔 만드는 것이죠. 이는 fastai에서 제공되는 기본 자료 형인 `ImageBlock`, `TextBlock` 등에 대해 `show_batch()` 또는 `show_result()`와 같은 작업을 처리하는 데 쓰이고 있습니다. 가령 주어진 입력 데이터가 이미지면 **이미지를** 화면에 출력하고, 텍스트라면 **텍스트를** 화면에 출력하게끔 서로다른 구현체를 가지지만 이를 처리하는 함수명은 모두 `show_batch`인 것이죠.

**타입 디스패치**를 사용하는 방법은 여러개 있으나, 가장 쉬운 방식은 `@typedispatch`라는 데커레이터를 사용하는 것입니다. `@typedispatch` 데커레이터가 붙은 함수 목록은 프로그램내 내부 딕셔너리에서 관리됩니다. 그리고 이 딕셔너리에서 특정 함수를 구분짓는 방법은 함수의 **처음 두 개의 파라미터**의 자료 형을 보는것이죠. 만약 하나만 주어지면, 두 번째 파라미터의 자료 형은 자동으로 **object**가 됩니다. 또 별도로 자료 형을 지정하지 않아도 **object**로 간주됩니다. 아래의 예를 살펴보죠.
