...
- **after_create**: `Learner`가 생성된 직후에 호출됩니다.
- **before_fit**: 전체 학습 또는 추론이 시작되기 전에 호출됩니다.
- **before_epoch**: 각 에포크의 시작에 호출됩니다.
- **before_train**: 각 에포크의 학습이 시작되기 전에 호출됩니다.
- **before_batch**: 각 배치가 주입되기 전에 호출됩니다.
- **after_pred**: 각 배치에 대한 모델의 예측이 계산된 후에 호출됩니다.
- **after_loss**: 각 배치에 대한 손실이 계산된 후에 호출됩니다(학습 외 상황 포함).
- **before_backward**: 각 배치에 대한 손실이 계산된 후에 호출됩니다(단, 역전파 전).
- **before_step**: 각 배치에 대한 역전파가 계산된 후에 호출됩니다(단, 파라미터 갱신 전).
- **after_step**: 각 배치에 대한 역전파에 따른 파라미터가 갱신된 후에 호출됩니다.
- **after_batch**: 각 배치의 주입이 완료된 후에 호출됩니다.
- **after_train**: 각 에포크의 학습이 끝난 후에 호출됩니다.
- **before_validate**: 각 에포크의 검증 계산이 수행되기 전에 호출됩니다.
- **after_validate**: 각 에포크의 검증 계산이 수행된 후에 호출됩니다.
- **after_epoch**: 각 에포크 끝에 호출됩니다.
- **after_fit**: 전체 학습 또는 추론이 끝난 후에 호출됩니다.
...
<exercise id="9" title="데이터 블록 API로 입력 데이터 구성하기">

fastai 에서는 입력 데이터를 구성하는 다양한 방식을 제공합니다. 앞서 본 `ImageDataLoaders` 클래스가 제공하는 여러 팩터리 메서드를 가장 고수준으로 추상화된 API로 볼 수 있습니다. 틀에 정확히 들어맞는 문제를 해결할 때는 매우 유용하죠. 단 한 줄이면 해결되니까요. 

하지만 때로는 입력 데이터 형식이 특정 틀에 완전히 들어맞는 경우는 매우 드뭅니다. 따라서 어느정도 커스터마이징이 필요합니다. 이렇게 입력 데이터를 구성하는 방식 중 하나는 fastai의 데이터 블록 API를 사용하는 것입니다. 

2장의 첫 번째 섹션에서 살펴봤던 `ImageDataLoaders` 객체의 `from_name_func` 라는 팩터리 메서드가 하는 일을 **데이터 블록 API**로 만들어보며 사용법을 익혀보겠습니다. 근본적으로는 `from_name_func` 메서드와 동일합니다. 왜냐하면 `from_name_func` 메서드 내부는 **데이터 블록 API**를 사용해 `DataLoaders` 객체를 반환하거든요.
...
<exercise id="10" title="데이터 블록 API의 기본 파라미터 파악하기">

실행 결과가 2장 1섹션에서와 정확히 같은것을 알 수 있습니다. 각 파라미터에 들어가는 구체적인 값이 하는 일은 차차 알아가게 될 것입니다. 이번에는 우선 각 파라미터의 의미를 살펴보겠습니다. 우선 `DataBlock` 객체는 데이터를 어쩌구저쩌구 처리해서, 모델이 학습할 수 있는 최종 형태로 가공하는 **일종의 설계 도면** 이라고 생각하면 좋습니다. 도면만 만들어두는 것이고, 이 도면에 실제 데이터를 흘려 보내서 출력 결과를 얻는것은 나중입니다. 

그러면 설계 도면을 그리는 데 필요한 구성 요소를 살펴보죠. 파라미터의 나열 순서와는 상관 없습니다.

**get_items**
- 최초로 입력 받을 데이터 목록을 반환하는 함수를 지정합니다. `get_image_files` 함수는 특정 경로내 모든 이미지 파일을 불러오는 기능을 했었죠. 다만 여기서는 실제 호출하지 않고, 함수 그 자체를 참조로 지정하였습니다. 설계 도면이기 때문이죠.

**get_y**
- `get_items`에 할당된 함수가 반환하는 리스트의 각 목록에서, 모델의 레이블로 활용할 부분을 추출(반환)하는 기능의 함수를 지정합니다. `get_image_files` 함수는 이미지 파일들의 `Path` 목록이므로, 각 `Path`로부터 파일명에 접근하여 첫 문자의 대/소문자 여부에 따라 **개** 또는 **고양이** 라는 레이블을 반환합니다. 

**get_x**
- `get_x`도 있으나 생략되었습니다. 생략되는 경우, `get_items`에 할당된 함수가 반환하는 리스트의 각 목록이 됩니다. 이게 디폴트 작동 방식이죠. 하지만 때로는 추가적인 작업이 필요할 수 있습니다. 가령 데이터프레임내 이미지 전체 경로 대신, 파일명만 제공되는 경우, 실제 파일의 전체 경로로 만들어 반환하는 작업이 필요할 수 있습니다.

**blocks**
- `get_items` -> `get_x`, `get_x` 파라미터에 지정된 함수들을 통해, 어떤 데이터를 쓰겠다는 파악이 되었습니다. 하지만 여전히 모델에 주입될 수는 없는 형태죠. 문자열 또는 `Path` 객체 입니다. 이를 모델이 이해할 수 있는 `Tensor` 자료형으로 변환해줄 필요가 있습니다. 이 작업이 수행되는 곳이 바로 `blocks` 파라미터 입니다. 
- `(ImageBlock, CategoryBlock)` 튜블의 각 요소는 각각 `get_x`, `get_y`가 반환한 값에 적용됩니다. 1:1 관계죠. 물론 다중 입력, 다중 레이블을 구성하는것도 가능합니다. 이는 추후 알아봅니다. 
- 기본적으로 `ImageBlock`은 파일 경로로 부터 이미지 비트맵을 PIL 객체로 불러오고, 이를 `Tensor` 자료형으로 변환하는 작업을 수행합니다. `CategoryBlock`은 해당 레이블을 표현하는 숫자를 할당하고(단어장 이라는 의미로 `vocab`을 자동 구성), 이를 `Tensor` 자료형으로 변환합니다.
